...
class BiliBiliSessionCrypto private constructor(private val key: SecretKeySpec) {
    ...
    companion object {
        ...
        private val AAD = "typetype.bilibili-session".toByteArray(Charsets.UTF_8)
        private val encoder = Base64.getUrlEncoder().withoutPadding()
        private val decoder = Base64.getUrlDecoder()

        fun fromSecret(secret: String): BiliBiliSessionCrypto {
            require(secret.length >= MIN_SECRET_LENGTH) { "BiliBili session encryption key is too short" }
            val digest = MessageDigest.getInstance("SHA-256")
                .digest(secret.toByteArray(Charsets.UTF_8))
            return BiliBiliSessionCrypto(SecretKeySpec(digest, "AES"))
        }
    }
}
