...
class AdminBugReportStatusRoutesTest {
    ...
    @Test
    fun `PUT admin bug report status rejects skipped transition`() = testApplication {
        val created = service.create(TEST_USER_ID, request())
        application {
            install(ContentNegotiation) { json() }
            routing { adminBugReportRoutes(adminAuth, service, fakeIssueService()) }
        }
        val response = client.put("/admin/bug-reports/${created.id}/status") {
            header(HttpHeaders.Authorization, "Bearer test-jwt")
            contentType(ContentType.Application.Json)
            setBody("""{"status":"fixed"}""")
        }
        assertEquals(HttpStatusCode.BadRequest, response.status)
    }

    @Test
    fun `PUT admin bug report status allows next transition`() = testApplication {
        val created = service.create(TEST_USER_ID, request())
        application {
            install(ContentNegotiation) { json() }
            routing { adminBugReportRoutes(adminAuth, service, fakeIssueService()) }
