...
class HomeRecommendationShortsDebugRoutesTest {
    ...
    fun `shorts endpoint returns source debug payload when enabled`() = testApplication {
        ...
        assertTrue(body.contains("\"debug\":{"))
        assertTrue(body.contains("\"sourceBreakdown\":{"))
        assertTrue(body.contains("\"itemSources\":{"))
    }
